...
//   const { workerId } = useParams();
  ...
  // In WorkerProfile.tsx - fix the fetch URL
useEffect(() => {
  const fetchWorkerProfile = async () => {
    try {
      setLoading(true);
      console.log('🔍 Fetching worker profile for:', workerId);
      
      // Try both endpoints - one for user_id and one for profile_id
      ...
      if (!response.ok) {
        // If first attempt fails, try the smart endpoint
        const smartResponse = await fetch(`https://verinest.up.railway.app/api/labour/workers/${workerId}/smart`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
        ...
        if (smartResponse.ok) {
          ...
          console.log('✅ Worker profile data (smart):', smartData);
          setWorker(smartData.data);
        } else {
          throw new Error(`Failed to fetch worker profile: ${smartResponse.status}`);
        }
      } else {
